...
if [ "$KERNEL_VERSION" = "6.8.0" ]; then
  for dir in ds_benchmark apache leveldb lmbench metis microbench psearchy parsec-benchmark; do
    (cd "$dir" && sudo python3 bench.py)
  done
fi

if [ "$KERNEL_VERSION" = "6.8.0-interval-vm+" ]; then
  for dir in apache leveldb lmbench metis microbench psearchy parsec-benchmark; do
    (cd "$dir" && sudo python3 bench.py)
  done
fi
