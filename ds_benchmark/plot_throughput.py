...
version_info = {
    "isl": {"label": "Interval Skiplist", "linestyle": "solid", "marker": "s", "color": "blue"},
    "mp": {"label": "Maple Tree", "linestyle": "solid", "marker": "x", "color": "red"},
}

benchmarks = ["alloc", "find", "remove"]
benchmark_info = {
    "alloc": {"title": "Alloc", "unit": 1000000, "unittype": "M"},
    "find": {"title": "Query", "unit": 1000000, "unittype": "M"},
    "remove": {"title": "Map", "unit": 1000000, "unittype": "M"},
}

def load_csv_files(benchtype):
...
def draw_one(benchtype, output_name, data, cpu_count=48):
    ...
    plt.ylabel("Throughput (" + benchmark_info[benchtype]["unittype"] + " op/s)", fontsize=label_size)
...
def draw(cpu_count=48):
    for benchtype in benchmarks:
        data = load_csv_files("results" + "/" + benchtype)
